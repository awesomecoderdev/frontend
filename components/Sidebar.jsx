	...
	return (
		<div className="relative overflow-x-hidden lg:w-80 w-auto h-auto min-h-screen  border-r border-slate-300/50 ">
			...
			<div className="relative pt-0 md:p-4 p-2 text-slate-700 ">
				<motion.nav
					className=" relative grid space-y-3 font-semibold font-inter "
					variants={{
						hidden: {
							opacity: 1,
							scale: 0,
						...
						<Fragment key={item.name}>
							<Link href={item.href} passHref={true}>
								<motion.a
									className={` ${
										route == item.href
											? "lg:bg-slate-100 bg-slate-200"
											: "lg:bg-slate-50/10 bg-slate-100/70"
									} transition md:p-3 p-2 duration-150 relative cursor-pointer hover:bg-slate-100 rounded-lg flex lg:justify-start justify-center items-center`}
									variants={{
										hidden: {
											x: -10,
											opacity: 0,
								...
								>
									<item.icon
										...
										aria-hidden="true"
									/>
									<span className="text-xs lg:block hidden ml-3 pointer-events-none text-slate-500">
										{item.name}
									</span>
								</motion.a>
							</Link>
