        ...
        async function verifyEmail() {
            try {
                if(params && userid && hash && expires && signature){
                    const verify = "/user/verify-email/"+userid+"/"+hash+"?expires="+expires+"&signature="+signature;
                        ...
                        if(response.success){
                            ...
                            if(response.status === 429){ // Too many attempts.
                                ...
                                setNotification({
                                    ...
                                    message: "Somethings went wrong. Please try again after sometimes."
                                })
                            }else if(response.status === 403){ // Invalid signature.
                                // console.log("Invalid signature.");
                                setErr("Your verification link has expired.")
                                setNotification({
                                    success: "false",
                                    title: "Invalid verification link!",
                                    message: "Your verification link has expired. Please request for a new verification email which contains a confirmation link."
                                })
                            }else{
                                setNotification({
                                    success: "false",
                                    title: "Unauthenticated!",
                                    message: "Pleas login and try again after sometimes."
                                })
                                // router.push("/login")
                            }
                ...
                <div className="relative bg-white max-w-md w-screen p-5 py-10 rounded-lg flex-col flex justify-center">
                    <div className="relative flex justify-center items-center mb-5">
                        <div className="bg-primary-100 h-12 w-12 rounded-full p-3 flex justify-center items-center">
                            <EnvelopeIcon strokeWidth={1.3} className="h-8 w-8 pointer-events-none text-primary-600" aria-hidden="true"  />
                        </div>
                    </div>
                    <div className="relative">
                        <h1 className="mb-2 md:mb-3 text-2xl font-medium text-center text-slate-600">Check your email</h1>
                    </div>
                    <p className='text-base md:text-lg text-slate-600 text-center mb-8'>We We sent a verification link to your email</p>
                    <button onClick={(e)=> sendVerificationEmail(e)} className="max-w-xs mx-auto my-4 focus-visible:outline-none w-full flex justify-center items-center text-white bg-primary-700 hover:bg-primary-800 focus:ring-4 focus:outline-none focus:ring-primary-300        font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-primary-600 dark:hover:bg-primary-700 dark:focus:ring-primary-800">
                        {loading ?
                            <>
